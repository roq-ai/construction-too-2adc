...
import { deleteRentalTransactionById } from 'apiSdk/rental-transactions';
...
function CustomerViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CustomerInterface>(
    ...
    () =>
      getCustomerById(id, {
        relations: ['user', 'rental_transaction'],
      }),
  );

  const rental_transactionHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('rental_transaction', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Rental Transactions
                    </Text>
                    <NextLink passHref href={`/rental-transactions/create?customer_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.rental_transaction?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/rental-transactions/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess(
                                ...
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/rental-transactions/edit/${record.id}`}
                                >
                              ...
                              {hasAccess(
                                ...
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    rental_transactionHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
